...
class _TenantPaymentsScreenState extends State<TenantPaymentsScreen> {
  ...
  Future<void> _savePayments() async {
    ...
    List<Payment> all = [];
    if (allJson != null && allJson.isNotEmpty) all = (jsonDecode(allJson) as List).map((e) => Payment.fromJson(e)).toList();
  ...
  Future<void> _sharePDFWithAttachment(Payment payment) async {
    ...
    if (file == null) return;
    try {
      await Share.shareXFiles([XFile(file.path)], text: 'Quittance de loyer - ${_formatMonth(payment.dueDate)}', subject: 'Quittance de loyer');
    } catch (e) {
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text('Quittance de loyer', style: GoogleFonts.urbanist(fontSize: 20, fontWeight: FontWeight.w600)),
            const SizedBox(height: 8),
            Text('Période : ${_formatMonth(payment.dueDate)}', style: GoogleFonts.urbanist(fontSize: 16)),
            Text('Montant : ${payment.formattedAmount}', style: GoogleFonts.urbanist(fontSize: 14, color: Colors.grey)),
            const SizedBox(height: 24),
            Text('📎 Partager le PDF', style: GoogleFonts.urbanist(fontSize: 14, fontWeight: FontWeight.w600)),
            const SizedBox(height: 8),
            Row(
              children: [
                Expanded(child: _buildSendButton(icon: Icons.email, label: 'Email', color: Colors.blue, onPressed: () async { Navigator.pop(context); await _sharePDFWithAttachment(payment); })),
                const SizedBox(width: 16),
                Expanded(child: _buildSendButton(icon: Icons.message, label: 'WhatsApp', color: Colors.green, onPressed: () async { Navigator.pop(context); await _sharePDFWithAttachment(payment); })),
              ],
            ),
            const SizedBox(height: 20),
            Text('💬 Ouvrir directement', style: GoogleFonts.urbanist(fontSize: 14, fontWeight: FontWeight.w600)),
            const SizedBox(height: 8),
            Row(
              children: [
                Expanded(child: _buildSendButton(icon: Icons.mail_outline, label: 'Ouvrir Mail', color: Colors.indigo, onPressed: () async { Navigator.pop(context); await _openMailApp(payment); })),
                const SizedBox(width: 16),
                Expanded(child: _buildSendButton(icon: Icons.chat_bubble_outline, label: 'Ouvrir WhatsApp', color: Colors.teal, onPressed: () async { Navigator.pop(context); await _openWhatsApp(payment); })),
              ],
            ),
            const SizedBox(height: 16),
            TextButton(onPressed: () => Navigator.pop(context), child: Text('Fermer', style: GoogleFonts.urbanist(color: Colors.grey))),
          ],
        ),
      ),
    );
      ...
      builder: (context) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Période : ${_formatMonth(payment.dueDate)}', style: GoogleFonts.urbanist(fontSize: 16)),
            Text('Montant : ${payment.formattedAmount}', style: GoogleFonts.urbanist(fontSize: 14, color: Colors.grey)),
            Text('Payé le : ${payment.paymentDate!.day}/${payment.paymentDate!.month}/${payment.paymentDate!.year}', style: GoogleFonts.urbanist(fontSize: 14, color: Colors.green)),
            const SizedBox(height: 24),
            Row(
              children: [
                Expanded(child: _buildSendButton(icon: Icons.picture_as_pdf, label: 'Ouvrir PDF', color: Colors.red, onPressed: () async { Navigator.pop(context); final file = await _generatePDF(payment); if (file != null) await PdfService.openPDF(file); })),
                const SizedBox(width: 16),
                Expanded(child: _buildSendButton(icon: Icons.share, label: 'Partager', color: Colors.blue, onPressed: () async { Navigator.pop(context); await _sharePDFWithAttachment(payment); })),
              ],
            ),
            const SizedBox(height: 16),
            TextButton(onPressed: () => Navigator.pop(context), child: Text('Fermer', style: GoogleFonts.urbanist(color: Colors.grey))),
  ...
  Widget _buildSendButton({required IconData icon, required String label, required Color color, required VoidCallback onPressed}) {
    return ElevatedButton.icon(
      onPressed: onPressed,
      icon: Icon(icon, size: 20, color: Colors.white),
      label: Text(label),
      style: ElevatedButton.styleFrom(backgroundColor: color, padding: const EdgeInsets.symmetric(vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
    );
  }
